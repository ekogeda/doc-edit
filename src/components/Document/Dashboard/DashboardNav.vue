	...
	<div class="main-menu menu-fixed menu-light menu-accordion menu-shadow">
		...
		<div class="main-menu-content">
			<ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
				...
				<li class="nav-item has-sub sidebar-group-active" :class="{ open: !isDropdownOpen }">
					...
					<ul class="menu-content ms-3 mt-1 border-start">
						<li class="d-none" :class="{ active: isPage == 'Files', disabled: dashboard.isLoadTable }">
							<a class="d-flex align-items-center" role="button" @click="page('Files')">
								<i data-feather="folder-plus"></i>
								<span class="menu-item text-truncate">My Files</span>
							</a>
						</li>
						<li :class="{ active: isPage == 'Completed', disabled: dashboard.isLoadTable }">
							<a class="d-flex align-items-center disabled" role="button" @click="page('Completed')">
								<span class="menu-item text-truncate">Completed</span>
								<template v-if="isEmptyArray(completedDocuments.data) || totalCompletedDocuments >= 0">
              ...
              <a class="d-flex align-items-center" role="button" @click="page('Locked')">
                ...
                <span class="badge badge-light-dark rounded-pill ms-auto me-0">
                  {{ lockerCounter }}
						...
						<li :class="{ active: isPage == 'Received', disabled: dashboard.isLoadTable }">
							<a class="d-flex align-items-center" role="button" @click="page('Received')">
								<span class="menu-item text-truncate">Need to sign</span>
								<template v-if="isEmptyArray(receivedDocuments.data) || totalReceivedDocuments >= 0">
									<span class="badge badge-light-dark rounded-pill ms-auto me-0">
						...
						<li :class="{ active: isPage == 'Declined', disabled: dashboard.isLoadTable }">
							<a class="d-flex align-items-center" role="button" @click="page('Declined')">
								<span class="menu-item text-truncate">Declined</span>
								<template v-if="isEmptyArray(declinedDocuments.data) || totalDeclinedDocuments >= 0">
									<span class="badge badge-light-dark rounded-pill ms-auto me-0">
						...
						<li>
							...
							<a class="d-flex align-items-center" role="button">
								...
								<span title="Coming Soon" class="badge badge-light-warning coming-soon border border-warning ms-auto"
									>Coming Soon</span
								>
							</a>
						</li>
			...
			<div class="help-box help-box-light text-center">
				...
				<div v-if="envelopLeft" style="font-size: 0.9rem">Sends remaining: {{ envelopLeft }}</div>

				<template v-if="dashboard.plan == 'Basic' || dashboard.plan == ''">
					<div style="font-size: 0.9rem">
						<p class="mb-2">Upgrade to <span class="text-primary fw-bolder">Pro</span> to share more documents.</p>
						<div class="text-center">
...
const { token, teams, statistics, receivedDocuments, declinedDocuments, completedDocuments } = useGetters({
	token: 'auth/token',
	teams: 'team/teams',
...
const envelopLeft = computed(() => {
	teams.value.map((team) => {
		if (team.active === true && team.subscription != null) {
			...
			numberOfEnvelopLeft.value = remainingEnvelops('number of envelops', hasPlan.features);
		}
	});
