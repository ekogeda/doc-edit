<template>
  <div class="grid">
    <div v-for="(print, index) in prints.CompanySeal" :key="index" class="border" :for="print.id">
      <template v-if="print.user_id">
        <div class="position-relative">
          <img :src="print.file" class="img-fluid" width="200" :alt="print.id" height="30" />
          <a role="button" @click="deleteSeal(print.id)" class="text-danger btn-close d-block text-end delete"></a>
        </div>
      </template>
